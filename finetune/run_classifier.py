...
def evaluate(args, dataset, istest = False):
    ...
    if istest:
        ...
        eps = 1e-9
        filename = ""
        filename = args.output_model_path[:-19]
        filename = filename + "prf/prf.csv"
        f2 = open(filename,"w+")
        f2.write('Label num')
        ...
        # 输出实验结果
        #wandb.log({"AC": correct / len(dataset)})
        #wandb.log({"PR": sum(Precision_list) / len(Precision_list)})
        #wandb.log({"RC": sum(Recall_list) / len(Recall_list)})
        #wandb.log({"F1": sum(F1_list) / len(F1_list)})
        f2.close()
...
def main():
    ...
    parser.add_argument("--labels_num", type=int,
                        help="labels_num")
    parser.add_argument("--project_name", type=str, default="DCS",
                        help="name of project")
    parser.add_argument("--name", type=str, default="ID = 4",
    ...
    for epoch in tqdm.tqdm(range(1, args.epochs_num + 1)):
        args.logger.info(args.epochs_num)
    ...
    if args.test_path is not None:
        ...
        evaluate(args, read_dataset(args, args.test_path),istest = True)
